import { PARTS, PlacedPart, PartKind } from './parts'
...
function isSurfaceKind(kind: PartKind | undefined): boolean {
  return kind === 'surface' || kind === 'cushion'
}
...
export function validatePhysics(parts: PlacedPart[]): PhysicsResult {
  ...
  const surfaces = parts.filter(p => isSurfaceKind(PARTS[p.type]?.kind))
  ...
  if (!parts.length) return {
    stable: false, score: 0, reasons: ['No parts'],
    com: {x:0,y:0,z:0}, legCount:0, surfaceCount:0, perPartStatus
  }

  let tm = 0, cx = 0, cy = 0, cz = 0
  parts.forEach(p => {
    const d = PARTS[p.type]; if (!d) return
    tm += d.kg; cx += p.x*d.kg; cy += p.y*d.kg; cz += p.z*d.kg
  })
  ...
  for (const surf of surfaces) {
    ...
    if (sLegs.length >= 3 || sPanels.length >= 2) {
      ...
      if (sLegs.length > 1) {
        ...
        if (Math.max(...bots) - Math.min(...bots) > 0.08) {
          reasons.push('Unequal leg heights'); score -= 25
          perPartStatus[surf.id] = 'unstable'
          sLegs.forEach(l => { perPartStatus[l.id] = 'unstable' })
        }
      }

      if (sLegs.length >= 2) {
        const lx = sLegs.map(l => l.x), lz = sLegs.map(l => l.z)
        if (cx < Math.min(...lx)-0.05 || cx > Math.max(...lx)+0.05 ||
            cz < Math.min(...lz)-0.05 || cz > Math.max(...lz)+0.05) {
          reasons.push('CoM outside base'); score -= 30
          perPartStatus[surf.id] = 'unstable'
        }
      }
    } else {
    ...
    backs.forEach(b => {
      const ok = Math.abs(b.x - surf.cx) < (sb.maxX-sb.minX)/2+0.1 &&
                 Math.abs(b.z - surf.cz) < 0.3
  ...
  parts.forEach(p => {
    if (perPartStatus[p.id] === 'neutral') {
      const b = getBounds(p)
      if (b && b.minY < 0.04) perPartStatus[p.id] = 'stable'
    }
  })
