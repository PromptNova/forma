...
function Label({ children }: { children: React.ReactNode }) {
    return (
          <div style={{
                  ...
                  color: 'var(--t3)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8,
          }}>{children}</div>div>
        )
}
...
function Row({ label, value, color }: { label: string; value: string | number; color?: string }) {
    return (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: '0.5px solid var(--bd)' }}>
                  <span style={{ fontSize: 12, color: 'var(--t2)', fontFamily: 'Inter, sans-serif' }}>{label}</span>span>
                  <span style={{ fontSize: 12, fontFamily: 'DM Mono, monospace', color: color || 'var(--t)', fontWeight: 500 }}>{value}</span>span>
          </div>div>
        )
}
...
function NumberInput({ label, value, onChange, step = 0.01, unit = '' }: {
    ...
    return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                  <span style={{ width: 12, fontSize: 10, fontFamily: 'DM Mono, monospace', color: 'var(--t3)', fontWeight: 600 }}>{label}</span>span>
                  <input
                            type="number" step={step} value={parseFloat(value.toFixed(3))}
                            onChange={e => onChange(parseFloat(e.target.value))}
                            style={{
                                        flex: 1, padding: '4px 8px', height: 30, background: 'var(--p2)',
                                        border: '0.5px solid var(--bd)', borderRadius: 7, color: 'var(--t)',
                          ...
                          />
            {unit && <span style={{ fontSize: 10, color: 'var(--t3)', fontFamily: 'DM Mono, monospace' }}>{unit}</span>span>}
          </div>div>
        )
}
...
export default function PropsPanel() {
    const { parts, selectedId, selectPart, removePart, duplicatePart, updatePart, physics, customParts } = useFormaStore()
  ...
  // Check if this is a custom AI part
  const isCustomPart = selectedPart?.type?.startsWith('__custom__') ?? false
    const customPartId = isCustomPart ? selectedPart!.type.replace('__custom__', '') : null
    const customPartDef: CustomPart | undefined = customPartId
      ? customParts?.find((cp: CustomPart) => cp.id === customPartId)
          : undefined

  const totalKg = parts.reduce((a, p) => a + (PARTS[p.type]?.kg || 0), 0)
    const totalCost = parts.reduce((a, p) => a + (PARTS[p.type]?.price || 0), 0)
    const maxH = parts.length > 0
      ? Math.max(...parts.map(p => { const d = PARTS[p.type]; return d ? (p.y + d.h / 2) : 0 }))
          : 0

  return (
        <div style={{
                                                                                width: 220, height: '100%', background: 'var(--panel)',
                borderLeft: '0.5px solid var(--bd)', display: 'flex', flexDirection: 'column', flexShrink: 0,
        }}>
                <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
                          <AnimatePresence mode="wait">
                            {selectedPart && (selectedDef || isCustomPart) ? (
                      <motion.div
                                      key={selectedPart.id}
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                                                    <div>
                                                                      <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--t)', letterSpacing: '-0.3px', fontFamily: 'Inter, sans-serif' }}>
                                                                        {isCustomPart ? (customPartDef?.name || 'Custom Part') : selectedDef!.label}
                                                                      </div>div>
                                                                      <div style={{ fontSize: 11, color: 'var(--t3)', fontFamily: 'DM Mono, monospace', marginTop: 2 }}>
                                                                        {isCustomPart
                                                                                                ? `AI · ${customPartDef?.kind || 'custom'} · ${Math.round((customPartDef?.widthM || 0) * 100)}×${Math.round((customPartDef?.heightM || 0) * 100)}cm`
                                                                                                : `${selectedDef!.kind} · ${Math.round(selectedDef!.w*100)}×${Math.round(selectedDef!.h*100)}×${Math.round(selectedDef!.d*100)}cm`
                                                                        }
                                                                      </div>div>
                                                    </div>div>
                                                    <button onClick={() => selectPart(null)}
                                                                        style={{ background: 'none', border: 'none', color: 'var(--t3)', cursor: 'pointer', fontSize: 16, padding: 2, borderRadius: 4, lineHeight: 1 }}
                                                                      >×</button>button>
                                    </div>div>
                      
                        {/* AI badge for custom parts */}
                        {isCustomPart && (
                                                      <div style={{
                                                                          marginBottom: 14, padding: '6px 10px', borderRadius: 8,
                                                                          background: 'rgba(212,117,74,0.08)', border: '0.5px solid rgba(212,117,74,0.2)',
                                                                          fontSize: 11, color: 'var(--acc)', fontFamily: 'DM Mono, monospace',
                                                      }}>
                        ...
                        {physics && (
                                                      <div style={{
                                                                          ...
                                                                          background: physics.perPartStatus[selectedPart.id] === 'stable'
                                                                                                ? 'rgba(62,200,122,0.08)'
                                                                                                : physics.perPartStatus[selectedPart.id] === 'unstable'
                                                                                                ? 'rgba(224,82,82,0.08)' : 'var(--p2)',
                                                                          border: '0.5px solid ' + (
                                                                                                physics.perPartStatus[selectedPart.id] === 'stable' ? 'rgba(62,200,122,0.2)'
                                                                                                : physics.perPartStatus[selectedPart.id] === 'unstable' ? 'rgba(224,82,82,0.2)'
                                                                                                : 'var(--bd)'
                                                                                              ),
                                                        ...
                                                        {physics.perPartStatus[selectedPart.id] === 'stable' ? '✓ Stable'
                                                                              : physics.perPartStatus[selectedPart.id] === 'unstable' ? '⚠ Unstable'
                                                                              : '◦ Neutral'}
                                                      </div>div>
                                    )}
                        ...
                        {/* Position */}
                                    <div style={{ marginBottom: 16 }}>
                                      ...
                                      {(['x', 'y', 'z'] as const).map(axis => (
                                                        <NumberInput
                                                                              key={axis} label={axis.toUpperCase()}
                                                                              value={selectedPart[axis]}
                                                                              onChange={v => updatePart(selectedPart.id, { [axis]: v })}
                                                                              step={0.01} unit="m"
                                                                            />
                                                      ))}
                                    </div>div>
                        ...
                        {/* Properties */}
                                    <div style={{ marginBottom: 16 }}>
                                                    ...
                                                    <div style={{ background: 'var(--p2)', borderRadius: 10, padding: '2px 10px', border: '0.5px solid var(--bd)' }}>
                                                      {isCustomPart ? (
                                                          <>
                                                                                <Row label="Kind" value={customPartDef?.kind || '—'} />
                                                                                <Row label="Width" value={(customPartDef?.widthM ? Math.round(customPartDef.widthM * 100) : '—') + ' cm'} />
                                                                                <Row label="Height" value={(customPartDef?.heightM ? Math.round(customPartDef.heightM * 100) : '—') + ' cm'} />
                                                                                <Row label="Source" value="AI" color="var(--acc)" />
                                                          </>>
                                                        ) : (
                                                          <>
                        ...
                        {/* Actions */}
                                    <div style={{ display: 'flex', gap: 6 }}>
                                                    <button
                                                                        ...
                                                                        style={{
                                                                                              flex: 1, padding: '8px 0', background: 'var(--p2)', border: '0.5px solid var(--bd)',
                                                                                              borderRadius: 8, color: 'var(--t2)', fontSize: 12, cursor: 'pointer',
                                                    ...
                                                    <button
                                                                        ...
                                                                        style={{
                                                                                              flex: 1, padding: '8px 0', background: 'rgba(224,82,82,0.07)',
                                                                                              border: '0.5px solid rgba(224,82,82,0.2)', borderRadius: 8,
                  ...
                  {parts.length > 0 && (
                    <div style={{ marginTop: 16 }}>
                                <Label>Bill of Materials</Label>Label>
                                <div style={{
                                    background: 'var(--p2)', borderRadius: 10, padding: '2px 10px',
                                    border: '0.5px solid var(--bd)', maxHeight: 160, overflowY: 'auto'
                    }}>
                                  {parts.map(p => {
                                      const def = PARTS[p.type]
                                                        const isAI = p.type.startsWith('__custom__')
                                                                          const aiDef = isAI ? customParts?.find((cp: CustomPart) => cp.id === p.type.replace('__custom__', '')) : null
                                                                                            const label = isAI ? (aiDef?.name || 'AI Part') : (def?.label)
                                                                                                              const price = isAI ? (aiDef?.priceEur ? '€' + aiDef.priceEur : 'AI') : (def ? '€' + def.price : '—')
                                                                                                                                if (!label) return null
                                                                                                                                                  return (
                                                                                                                                                                      <div key={p.id} onClick={() => selectPart(p.id)}
                                                                                                                                                                                            style={{
                                                                                                                                                                                          ...
                                                                                                                                                                                          <span style={{ fontSize: 11, color: 'var(--t3)', fontFamily: 'DM Mono, monospace' }}>{price}</span>span>
                                                                                                                                                                        </div>div>
                                                                                                                                                                    )
                                  })}
                                </div>div>
          ...
          { label: 'Cost', value: '€' + totalCost.toFixed(0) },
                              ...
                              <div key={s.label} style={{ textAlign: 'center' }}>
                                          <div style={{ fontSize: 9, color: 'var(--t3)', fontFamily: 'Inter, sans-serif', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 2 }}>{s.label}</div>div>
                                          <div style={{ fontSize: 13, fontFamily: 'DM Mono, monospace', fontWeight: 600, color: 'var(--t)' }}>{s.value}</div>div>
                              </div>div>
                            ))}
              </div>div>
        </div>div>
